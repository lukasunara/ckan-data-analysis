...
if (findChart.max_id > 0) {
    let configID = getDoughnutChar(
        findChart.identification, findChart.max_id, 'Identification',
...
if (findChart.max_key > 0) {
    let configKey = getDoughnutChar(
        findChart.keywords, findChart.max_key, 'Keywords',
        findChart.darkColor, findChart.lightColor
    );
...
if (findChart.max_cat > 0) {
    let configCategories = getDoughnutChar(
        findChart.categories, findChart.max_cat, 'Categories',
        findChart.darkColor, findChart.lightColor
    );
...
if (findChart.max_state > 0) {
    let configState = getDoughnutChar(
        findChart.state, findChart.max_state, 'State',
        findChart.darkColor, findChart.lightColor
    );
